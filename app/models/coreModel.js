...
class CoreModel {
  ...
  static findBy(params, callback) {
    ...
    const param = Object.getOwnPropertyNames(params)
    ...
    const query = {
      text: `SELECT * FROM "${this.tableName}" WHERE "${paramString}" = '${valueString}';`,
      // values: [paramString, valueString]
    }
  ...
  static save(callback) {
    // on veut appeler soit update soit insert si l'instance existe déjà dans la BDD ou pas
    // const save = 
    // il faut qu'on cherche si l'instance existe déjà dans la BDD avec peut être un .find 

    // si ça existe on update

    // si ça n'existe pas on insert
